(function(){
  ...
  function createRoot(){
    var old=$('#__bupt_empty_classroom_bookmarklet'); if(old)return old;
    var root=D.createElement('div'); root.id='__bupt_empty_classroom_bookmarklet';
    root.innerHTML='<style>'+cssText()+'</style><div class="ec-app"><div class="ec-top"><button class="ec-link" id="ec-back">返回官方页面</button><button class="ec-link" id="ec-refresh">重读数据</button><button class="ec-link" id="ec-close">关闭</button></div><div class="ec-title"><div class="ec-logo">BUPT</div><h1>BUPT 空教室查询</h1><p>数据来自你已打开的官方空闲教室页面；本脚本不读取 Cookie，不保存账号密码，不上传数据。西土城仅教一/教二/教三/教四/未来学习大楼。</p></div><div id="ec-main"></div><div class="ec-footer">官方页面增强显示 · '+esc(VERSION)+'</div></div>';
    D.body.appendChild(root);
    $('#ec-back',root).onclick=function(){root.style.display='none'};
    $('#ec-close',root).onclick=function(){root.remove()};
    $('#ec-refresh',root).onclick=function(){state.loading=true;render(); if(!syncFromVue()){state.error='没读到页面数据，请先在官方页面进入“空闲教室”，或返回官方页面切换一次校区。'} state.loading=false;render();};
    return root;
  }
  function cssText(){return ''+
    ...
    '#__bupt_empty_classroom_bookmarklet .ec-btn{border:1px solid #d9d9d9;background:#fff;color:#111;border-radius:0;min-width:6em;margin:3px;padding:6px 10px;}'+
    '#__bupt_empty_classroom_bookmarklet .ec-btn.active{background:#1677ff;color:#fff;border-color:#1677ff;}'+
    '#__bupt_empty_classroom_bookmarklet .ec-time{width:45px;height:45px;border:1px solid #d9d9d9;background:#fff;border-radius:0;margin:2px;padding:0;line-height:1.05;vertical-align:top;font-size:12px;}'+
    ...
    '#__bupt_empty_classroom_bookmarklet .ec-small{border:1px solid #d9d9d9;background:#fff;border-radius:0;margin:3px;padding:6px 10px;}#__bupt_empty_classroom_bookmarklet .ec-small.active{background:#1677ff;color:#fff;border-color:#1677ff;}'+
    ...
    '@media(max-width:700px){#__bupt_empty_classroom_bookmarklet .ec-app{padding:8px;}#__bupt_empty_classroom_bookmarklet .ec-card{padding:10px;}#__bupt_empty_classroom_bookmarklet .ec-btn{min-width:5em;}#__bupt_empty_classroom_bookmarklet th,#__bupt_empty_classroom_bookmarklet td{padding:8px 4px;font-size:12px;}}';}
  function render(){
    var root=createRoot(); root.style.display='block'; var main=$('#ec-main',root);
  ...
  function tableHtml(rows){return '<table><thead><tr><th>教室</th><th>座位数</th><th>类型</th><th>来源</th></tr></thead><tbody>'+rows.map(function(r,i){return '<tr><td><button class="ec-link" data-row="'+i+'">'+esc(r.name)+'</button></td><td>'+esc(r.size)+'</td><td>'+esc(r.type)+'</td><td><span class="ec-tag">教务</span></td></tr>'}).join('')+'</tbody></table>';}
